...
public abstract class PowerButton {
    ...
    public static final String BUTTON_MANUAL_BRIGHTNESS = "toggleManualBrightness";
...
//        BUTTONS.put(BUTTON_FLASHLIGHT, FlashlightButton.class);
    ...
    protected static final HashMap<String, PowerButton> BUTTONS_LOADED = new HashMap<String, PowerButton>();
...
*/
    ...
    protected void updateTextView(int id, int resId) {
        TextView imageText = (TextView)mView.findViewById(id);
        imageText.setText(resId);
    }
    ...
    public static boolean loadButton(String key, View view) {
        ...
        if((key == BUTTON_MANUAL_BRIGHTNESS || BUTTONS.containsKey(key)) && view != null) {
            synchronized (BUTTONS_LOADED) {
                if(BUTTONS_LOADED.containsKey(key)) {
                    ...
                    try {
                        ...
                        PowerButton pb = null;
                        if(key == BUTTON_MANUAL_BRIGHTNESS && BUTTONS_LOADED.containsKey(PowerButton.BUTTON_BRIGHTNESS)) {  
				pb = BrightnessAdjButton.class.newInstance();
			} else {
                        	pb = BUTTONS.get(key).newInstance();
			}
